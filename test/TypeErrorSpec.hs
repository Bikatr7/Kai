  ...
  describe "Arithmetic Type Errors" $ do
    it "rejects 1 + true" $ do
      case parseAndTypeCheck "1 + true" of
        ...
        _ -> expectationFailure "Should be unification error: TBool cannot unify with TInt"
    
    it "rejects true - 5" $ do
      case parseAndTypeCheck "true - 5" of
        Left (UnificationError TBool TInt) -> True `shouldBe` True
        _ -> expectationFailure "Should be unification error"
  ...
  describe "Boolean Logic Type Errors" $ do
    ...
    it "rejects not 42" $ do
      case parseAndTypeCheck "not 42" of
        Left (UnificationError TInt TBool) -> True `shouldBe` True
        _ -> expectationFailure "Should be unification error: TInt cannot unify with TBool"
  ...
  describe "Conditional Type Errors" $ do
    it "rejects if 5 then 1 else 2" $ do
      case parseAndTypeCheck "if 5 then 1 else 2" of
        Left (UnificationError TInt TBool) -> True `shouldBe` True
        _ -> expectationFailure "Should be unification error: condition must be bool"
    
    it "rejects if true then 1 else false" $ do
      case parseAndTypeCheck "if true then 1 else false" of
        Left (UnificationError TInt TBool) -> True `shouldBe` True
        _ -> expectationFailure "Should be unification error: branches must match"
